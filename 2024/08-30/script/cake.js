...
timeline().to('.cake__sprinkle', {
	scale: 1, duration: 0.06, stagger: 0.02
})
// Extract out to your own timeline
const SPIN_TL = () =>
timeline()
.set('.cake__frosting-patch', {
	display: 'block'
})
.to(
...
timeline({})
.to('.cake__candle', {
	'--flame': 1, stagger: 0.2, duration: 0.1
})
.to('body', {
	'--flame': 1, '--lightness': 5, duration: 0.2, delay: 0.2
})
const LIGHTS_OUT = () =>
timeline().to('body', {
...
.to('.char', {
	'--char-sat': 70, '--char-light': 65, duration: 0.2
}, 0)
.to('.char', {
